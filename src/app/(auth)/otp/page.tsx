...
function OtpVerificationPage() {
  ...
  const sendVerificationCodeRef = useRef(sendVerificationCode);
  ...
  useEffect(() => {
    if (!email || !role) {
      toast("Invalid email or role.");
      return;
    }

    sendVerificationCodeRef.current.mutate(
      { email, role },
      {
        onError: (error) => {
          toast(error.message);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-md border border-violet-900/50 bg-gradient-to-tl from-black via-black to-violet-950/30 rounded-lg p-8 shadow-xl">
        ...
        <p className="text-gray-400 text-center mb-6">We&apos;ve sent a verification code to</p>
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <button 
            onClick={resendCode}
            className="text-[#3b82f6] hover:text-[#2563eb] focus:outline-none"
          >
            Resend Code
          </button>
        </div>
