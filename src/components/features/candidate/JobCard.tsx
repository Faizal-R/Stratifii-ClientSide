...
import { Building, Eye, Ban, Clock, CheckCircle, XCircle, Play, AlertCircle } from "lucide-react";
...
const JobCard: React.FC<JobCardProps> = ({
  ...
  const getStatusBadge = () => {
    ...
    const config = statusConfig[job.mockStatus as keyof typeof statusConfig] || statusConfig.mock_pending;

    return (
      <div className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-medium border backdrop-blur-sm ${config.bg} ${config.border} ${config.text}`}>
        <div className={`w-2 h-2 rounded-full mr-3 ${config.dot} animate-pulse`}></div>
        {config.icon}
        <span className="ml-2 font-semibold">{formatStatus(job.mockStatus)}</span>
      </div>
    );
  };
  ...
  const getActionButton = () => {
    ...
    if (job.mockStatus === "mock_failed") {
      return (
        <button className="bg-gradient-to-r from-red-600/50 to-red-700/50 text-red-200 px-6 py-3 rounded-xl cursor-not-allowed font-semibold flex items-center space-x-2 opacity-70" disabled>
  ...
  return (
    <div className={`group relative overflow-hidden rounded-2xl transition-all duration-500 hover:scale-[1.02] ${cardVariant.hoverShadow}`}>
      {/* Background gradient */}
      <div className={`absolute inset-0 bg-gradient-to-br ${cardVariant.gradient} opacity-80`}></div>
      
      {/* Subtle pattern overlay */}
      <div className="absolute inset-0 bg-black/20 backdrop-blur-sm"></div>
      
      {/* Border */}
      <div className={`relative border ${cardVariant.border} ${cardVariant.hoverBorder} rounded-2xl transition-all duration-300`}>
